        ...
        -> Result<StructValue<'ctx>, CompileError>
    {
        ...
        if let Expr::Var(name, ..) = callee {
            ...
            if name == "getf" {
                ...
                // verb_struct_get expects the field name as a raw `char*`, not a
                // tagged value: a Verb string's payload already *is* a pointer to
                // its NUL-terminated bytes, so hand that pointer through directly.
                let key_ptr = self.builder
                    .build_int_to_ptr(self.payload_of(key), self.ptr_ty, "getf_key")
                    .unwrap();
                let (lc, cc) = self.loc_consts(line, col);
                let rv = self.call_named("verb_struct_get", &[s.into(), key_ptr.into(), lc.into(), cc.into()])
                    .unwrap().into_struct_value();
                self.call_named("verb_release_value", &[s.into()]);
            ...
            if name == "setf" {
                ...
                let key = self.gen_expr(&args[1])?;
                let key_ptr = self.builder
                    .build_int_to_ptr(self.payload_of(key), self.ptr_ty, "setf_key")
                    .unwrap();
                ...
                let (lc, cc) = self.loc_consts(line, col);
                let rv = self.call_named(
                    "verb_struct_set", &[s.into(), key_ptr.into(), v.into(), lc.into(), cc.into()])
                    .unwrap().into_struct_value();
                self.call_named("verb_release_value", &[s.into()]);
                self.call_named("verb_release_value", &[key.into()]);
                return Ok(rv);
