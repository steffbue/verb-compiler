...
pub fn parse(toks: Vec<Token>) -> Result<Program, CompileError> {
    ...
    Ok(Program { imports, std_imports, extern_sigs, verb_imports, body })
}
...
pub fn parse_recovering(toks: Vec<Token>) -> (Program, Vec<CompileError>) {
    ...
    (Program { imports, std_imports, extern_sigs, verb_imports, body }, errors)
}
...
impl Parser {
    ...
    fn imports(&mut self) -> Result<(Vec<String>, Vec<String>, Vec<ExternSig>, Vec<String>), CompileError> {
        let mut imports = Vec::new();
        let mut std_imports = Vec::new();
        let mut extern_sigs = Vec::new();
        let mut verb_imports = Vec::new();
        ...
        Ok((imports, std_imports, extern_sigs, verb_imports))
    }
    ...
    fn import_stmt(&mut self) -> Result<ImportStmt, CompileError> {
        ...
        // link time.
        if name != "io" && name != "map" && name != "net" && name != "thread" && name != "time" {
            return Err(CompileError::new(
                format!("unknown std module '{name}' (known std modules: io, map, net, thread, time)"),
                l, c,
            ));
        }
        self.expect(&TokenKind::Semi, "';'")?;
