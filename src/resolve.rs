...
impl Resolver {
    ...
    fn process(&mut self, path: &PathBuf, display: &str) -> Result<(), ResolveErrorKind> {
        ...
        // Typed FFI signatures are keyed by name in codegen (a HashMap), so a
        // duplicate name across files simply overwrites -- no dedup needed here.
        self.extern_sigs.extend(prog.extern_sigs);
